...
class NT_Settings_Page {
    ...
    public function render() {
        ...
        <div class="wrap">
            ...
            <h3><?php esc_html_e( 'Cleanup Tools', NT_TEXTDOMAIN ); ?></h3>
            <p><?php esc_html_e( 'Use this after v2.6.7 if old translated rows show NTP0, NTP1, NTP2, box characters, or other symbolic placeholder artifacts.', NT_TEXTDOMAIN ); ?></p>
            <form method="post">
                <?php wp_nonce_field( 'nt_clean_tokens' ); ?>
                <input type="hidden" name="nt_do" value="clean_tokens">
                <?php submit_button( __( 'Clean Existing Token Artifacts', NT_TEXTDOMAIN ), 'secondary' ); ?>
            </form>

            <h2><?php esc_html_e( 'Support Development', NT_TEXTDOMAIN ); ?></h2>
            <p><?php esc_html_e( 'If this plugin helps your multilingual publishing workflow, you can support continued development here:', NT_TEXTDOMAIN ); ?></p>
    ...
    public function handle_post_actions() {
        ...
        if ( 'test' === $nt_do ) {
        ...
        } elseif ( 'clean_tokens' === $nt_do ) {
            check_admin_referer( 'nt_clean_tokens' );
            $result = NT_DB::clean_existing_token_artifacts();
            if ( is_wp_error( $result ) ) {
                ...
                $this->set_notice(
                    'success',
                    sprintf(
                        /* translators: 1: rows checked, 2: rows updated */
                        __( 'Token cleanup complete. Checked %1$d translation row(s); updated %2$d row(s).', NT_TEXTDOMAIN ),
                        $checked,
                        $updated
                    )
                );
    ...
    private function ajax_subtitle_backfill_batch( $languages, $post_types, $batch_size ) {
        ...
        foreach ( $languages as $lang ) {
            ...
            foreach ( $rows as $row ) {
                ...
                $translated = NT_DB::clean_symbolic_tokens( $translated );
