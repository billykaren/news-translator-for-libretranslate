...
class NT_Post_Translator {
    ...
    private function restore_markup_tokens( $text, $tokens ) {
        if ( empty( $tokens ) ) {
            $text = preg_replace_callback('/(?:⟪NTP|ZZZPROT|NTPROTECTTOKEN|NTPROTECT)\s*(\d+)\s*(?:⟫|ZZZ)?/u', function($m) use ($tokens) {
        $num = $m[1];
        foreach ($tokens as $k => $v) {
            if (preg_match('/(\d+)/', $k, $km) && $km[1] == $num) return $v;
        }
        return $m[0];
    }, $text);
    ...
    private function translate_field( $text, $lang ) {
        ...
        return $this->restore_markup_tokens( $translated, $tokens );
    }
}
