...
class NT_Frontend {
    ...
    private static function clean_symbolic_tokens( $text ) {
        return class_exists( 'NT_DB' ) ? NT_DB::clean_symbolic_tokens( $text ) : $text;
    }
    ...
    public static function filter_subtitle( $value ) {
        ...
        if ( $lang && $post_id ) {
            ...
            if ( $row && ! empty( $row['subtitle'] ) ) {
                return self::clean_symbolic_tokens( $row['subtitle'] );
            }
        }
    ...
    public static function filter_subtitle_meta( $value, $object_id, $meta_key, $single ) {
        ...
        $row = NT_DB::get( $post_id, $lang );
        if ( $row && ! empty( $row['subtitle'] ) ) {
            return $single ? self::clean_symbolic_tokens( $row['subtitle'] ) : array( self::clean_symbolic_tokens( $row['subtitle'] ) );
        }
        ...
        if ( is_string( $original ) && trim( $original ) !== '' ) {
            return $single ? self::clean_symbolic_tokens( $original ) : array( self::clean_symbolic_tokens( $original ) );
        }
