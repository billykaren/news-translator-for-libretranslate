...
// ── Auto-translate on publish (v2.6.9: hybrid immediate publish + delayed subtitle finalizer) ─────────────────────
add_action( 'transition_post_status', 'nt_auto_translate_on_publish', 20, 3 );
add_action( 'wp_after_insert_post', 'nt_auto_translate_on_after_insert_fallback', 99, 4 );
add_action( 'nt_finalize_subtitle_translation', 'nt_finalize_subtitle_translation', 10, 1 );

/**
 * Main first-publish trigger.
 *
 * This restores the pre-v2.6.8 behavior for normal draft-to-publish workflows.
 */
function nt_auto_translate_on_publish( $new_status, $old_status, $post ) {
    if ( 'publish' !== $new_status || 'publish' === $old_status ) {
        return;
    }

    nt_run_auto_translate_for_post( $post, 'transition' );
}

/**
 * Fallback for editors/builders that bypass or delay normal status transitions.
 *
 * Do not require $update to be false. Existing drafts become published through an update,
 * so v2.6.8 skipped the most common publishing path.
 */
function nt_auto_translate_on_after_insert_fallback( $post_id, $post, $update, $post_before ) {
    if ( ! $post || 'publish' !== $post->post_status ) {
        return;
    }

    $old_status = ( $post_before && isset( $post_before->post_status ) ) ? $post_before->post_status : '';
    if ( 'publish' === $old_status ) {
        return;
    }

    nt_run_auto_translate_for_post( $post, 'after_insert' );
}

/**
 * Run immediate full translation once per published post.
 */
function nt_run_auto_translate_for_post( $post, $source = '' ) {
    ...
    if ( ! get_option( 'nt_auto_translate', 0 ) ) {
        return;
    }
    ...
    // Prevent duplicate work when both transition_post_status and wp_after_insert_post fire.
    $lock_key = 'nt_auto_translate_lock_' . $post_id;
    if ( get_transient( $lock_key ) ) {
        nt_schedule_subtitle_finalizer( $post_id );
        return;
    }
    ...
    foreach ( array_keys( NT_LANGS ) as $lang ) {
        $translator->get_or_create( $post_id, $lang );
    }

    update_post_meta( $post_id, '_nt_auto_translated_on_publish', current_time( 'mysql' ) );
    nt_schedule_subtitle_finalizer( $post_id );
}

/**
 * Schedule a subtitle-only finalizer after theme/editor meta saves have completed.
 */
function nt_schedule_subtitle_finalizer( $post_id ) {
    $post_id = (int) $post_id;
    if ( ! $post_id ) {
        return;
    }

    if ( ! wp_next_scheduled( 'nt_finalize_subtitle_translation', array( $post_id ) ) ) {
        wp_schedule_single_event( time() + 30, 'nt_finalize_subtitle_translation', array( $post_id ) );
    }
}

/**
 * Delayed subtitle-only pass.
 *
 * This keeps initial publish translation immediate, then safely updates the subtitle field
 * after ReviewNews/AFT meta is available.
 */
function nt_finalize_subtitle_translation( $post_id ) {
    $post_id = (int) $post_id;
    ...
    foreach ( array_keys( NT_LANGS ) as $lang ) {
        $translator->refresh_subtitle( $post_id, $lang );
    }
}
